 ...
 * WebDriver caps at 30 seconds. The waiting itself happens in NODE for the same reason: the 60s ceiling
 * this file used to declare inside a closure was one that cap could never honour, and it would have died
 * on exactly the cold phone the budget was for. The settles are Node-side sleeps now, a settle being
 ...
 * because a serialized callback cannot reach this module's scope, which is still true of the map below.
 *
